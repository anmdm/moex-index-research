"""
Optuna optimization of variant D (Group-TP) on SV (Silver) SOLO.
Compare with index (36%BR+28%NG+36%SV) to see if index has advantage.
"""
...
if __name__ == '__main__':
    t0 = time.time()
    print("=" * 70)
    print("  OPTUNA — Variant D on SV (Silver) SOLO")
    ...
    # SV solo: single asset
    sv_close = data[['SV']].values.astype(np.float64)
    sv_weights = np.array([1.0])
    is_session = mark_sessions(data.index)
    n_bars = len(sv_close)

    print(f'SV solo: {n_bars:,} bars')

    print('Computing Z-scores for SV solo...')
    ...
    print(f'\n{"="*70}')
    print(f'  SV SOLO — Variant D (Group-TP)')
    print(f'{"="*70}')
    ...
    # Comparison
    print(f'\n  --- COMPARISON ---')
    print(f'  Index (36%BR+28%NG+36%SV): +259% DD=-12.5% P&L/DD=20.7 yrs=[+24/+20/+124/+9]')
    print(f'  SV solo:                    {ret_pct:+.1f}% DD={dd_pct:.1f}% P&L/DD={abs(ret_pct/dd_pct) if dd_pct != 0 else 0:.1f} yrs=[{yr_str}]')

    result = {
        "strategy": "D: Group-TP on SV solo",
    ...
    with open('moex_sv_solo_results.json', 'w') as f:
        json.dump(result, f, indent=2, default=str)
    print(f'\nSaved: moex_sv_solo_results.json')
